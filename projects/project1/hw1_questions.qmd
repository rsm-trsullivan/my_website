...
$x_A$, $x_B$ denotes the number of **successes** (sign-ups)
...
To demonstrate this, we can simulate 1,000 draws from two Bernoulli distributions with our two probabilities. By doing this, we can mimic the randomness of real experiments in a controlled environment.
...
### Demonstrating the Bootstrap
To demonstrate this, we will carry out the following steps:
...
The standard error measures the sampling variability of the estimator, so a larger standard error leads to a wider interval, while a smaller standard error leads to a narrower interval. Thus, the confidence interval summarizes both the estimated treatment effect and the uncertainty associated with that estimate.

### Confidence Intervals with the Bootstrap Standard Error
We can now construct the confidence interval using the bootstrapped standard error:
...
We can demonstrate why peeking is problematic. Assume we have a world where the null hypothesis is true: $\pi_A = \pi_B = 0.20. $In a single test where $\alpha = 0.05$, if $H_0$ is true, we have a 5% chance of a false positive. When we test at at $n$ = 100, ... up to 1,000 visitors, each peek is risking crossing that 0.05 threshold. Repeating this at multiple levels, we increase or probability of false positives.

### Demonstrating Peeking
...
### Key Takeaways
1. An A/B test can be framed as a statistical problem of comparing two proportions, and the difference in sample means is a natural estimator for the treatment effect.
2. The Law of Large Numbers ensures that as the sample size grows, our estimator converges to the true parameter, giving us confidence in our estimates.
3. The bootstrap is a powerful tool for estimating the standard error of an estimator without relying on parametric assumptions, allowing us to construct confidence intervals that reflect our uncertainty.
4. The Central Limit Theorem justifies the use of Normal approximations for inference, enabling us to perform hypothesis tests and construct confidence intervals even when the underlying data are not Normally distributed.
5. The two-sample test for comparing means can be equivalently expressed as a linear regression, which provides a flexible framework for analyzing A/B tests and can foster more advanced regressions in which we can add controls, interactions, and multiple treatments.
6. Peeking at the data and stopping early when significance appears can greatly inflate the false positive rate, leading to a much higher chance of incorrectly declaring a winner when there is actually no difference between the groups.
